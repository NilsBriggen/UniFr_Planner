...
it("remembers applied course context per plan and scenario without overriding explicit links", () => {
  rememberCatalogueContext(
    ...
    "s",
    new URLSearchParams(
      "term=SS-2027&faculty=Psychology&language=fr&focus=all&offset=20",
    ),
  );
  expect(catalogueLinkFor("p", "s")).toBe(
    "/catalogue?term=SS-2027&faculty=Psychology&language=fr&focus=all&offset=20",
  );
